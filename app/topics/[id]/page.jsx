...
export default function TopicDetail() {
  ...
  const topic = {
    ...
    author: { name: 'Sarah Johnson', role: 'student' },
    category: 'Physical Chemistry',
    tags: ['bonding', 'basics', 'fundamentals'],
    stats: { likes: 45, posts: 23, views: 156 },
  ...
  const posts = [
    ...
    },
    {
      ...
      author: { name: 'Dr. Kumar', role: 'teacher' },
  ...
  return (
    <div className="min-h-screen">
      {/* Topic Header */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6 rounded-xl mb-6">
        {topic.isPinned && (
          <div className="flex items-center mb-4">
            <span className="px-3 py-1 bg-primary-500/20 text-primary-400 text-sm rounded-full flex items-center">📌 Pinned Topic</span>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          <span className="px-3 py-1 bg-white/10 text-primary-400 text-sm rounded-full">{topic.category}</span>
          {topic.tags.map((tag) => (
            <span key={tag} className="px-2 py-1 bg-white/5 text-gray-400 text-sm rounded">#{tag}</span>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className={`flex items-center space-x-1 transition-colors ${liked ? 'text-red-400' : 'text-gray-400 hover:text-red-400'}`}
            >
            ...
            <button
              ...
              className={`flex items-center space-x-1 transition-colors ${bookmarked ? 'text-yellow-400' : 'text-gray-400 hover:text-yellow-400'}`}
            >
      ...
      >
        ...
        <form onSubmit={handlePostSubmit}>
          ...
          <div className="flex justify-end mt-4">
            <button type="submit" disabled={!newPost.trim()} className="btn-primary flex items-center">
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-white mb-4">Discussion ({topic.stats.posts} posts)</h2>
        {posts.map((post, index) => (
          ...
          >
            ...
            <div className="flex items-center space-x-6 pt-4 border-t border-white/10">
              <button className={`flex items-center space-x-1 transition-colors ${post.isLiked ? 'text-red-400' : 'text-gray-400 hover:text-red-400'}`}>
