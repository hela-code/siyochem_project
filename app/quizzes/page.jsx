    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300 text-lg">Loading quizzes...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            ...
            <p className="text-gray-300">
              ...
              {pagination.total > 0 && (
                <span className="text-gray-500 ml-2">({pagination.total} quizzes)</span>
              )}
            </p>
          </div>
          {user?.role === 'teacher' && (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
          ...
          {user?.role === 'teacher' && (
            <Link href="/create-quiz" className="btn-primary mt-6 inline-flex items-center">
              <PlusCircle className="w-5 h-5 mr-2" /> Create the First Quiz
            </Link>
          )}
        </motion.div>
      )}
