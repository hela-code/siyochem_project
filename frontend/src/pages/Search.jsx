...
const SearchPage = () => {
    ...
    topics: [
      {
        ...
        description: 'Discussion on ionic, covalent, and metallic bonds',
    ...
    users: [
      {
        ...
        role: 'teacher',
        bio: 'Chemistry teacher with 5+ years experience',
  ...
  const getFilteredResults = () => {
    ...
    if (activeTab === 'all' || activeTab === 'topics') {
      allResults.push(...mockResults.topics.map(item => ({ ...item, type: 'topic' })))
    }
    if (activeTab === 'all' || activeTab === 'users') {
      allResults.push(...mockResults.users.map(item => ({ ...item, type: 'user' })))
    }
    if (activeTab === 'all' || activeTab === 'quizzes') {
      allResults.push(...mockResults.quizzes.map(item => ({ ...item, type: 'quiz' })))
    }
        ...
        {results.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((result, index) => {
              ...
              return (
                ...
                >
                  <Link to={getResultPath(result)}>
                    {/* Result Type Badge */}
                    <div className="flex items-center justify-between mb-3">
                      <span className="px-2 py-1 bg-primary-500/20 text-primary-400 text-xs rounded-full capitalize">
                        {result.type}
                      </span>
                    ...
                    {result.type === 'user' && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="px-2 py-1 bg-white/10 text-primary-400 text-xs rounded-full capitalize">
                          {result.role}
                        </span>
                      </div>
                    )}
          ...
          >
            ...
            <p className="text-gray-300">
              Enter keywords to find topics, users, or quizzes
            </p>
          </motion.div>
        )}
      </div>
        ...
        >
          <div className="glass-card p-6 rounded-xl">
            ...
            <div className="flex flex-wrap gap-2">
              {['Chemical Bonding', 'Organic Chemistry', 'Thermodynamics', 'Periodic Table', 'Reaction Mechanisms'].map((term, index) => (
                <button
                  key={index}
